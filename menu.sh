#!/bin/bash
# ==============================================================================
# ИНТЕРФЕЙС ГЛАВНОГО ЭКСПЕРТА (ОБОЛОЧКА ДЛЯ CORE.SH)
# ==============================================================================
...
while true; do
    ...
    if [ $? -ne 0 ] || [ "$MAIN_CAT" == "exit" ]; then
        echo -e "\n🐾 [БАГИР]: Системы переведены в ждущий режим. До связи, Вожак!"
        break
    fi

    # 🐾 БАГИР: Ветвление 2 уровня (Конкретные приказы)
    ACTION=""
    case "$MAIN_CAT" in
        deploy)
    ...
    if [ "$ACTION" == "reconf" ]; then
        if whiptail --title "СБРОС КОНФИГУРАЦИИ" --yesno "Вы собираетесь уничтожить текущий файл $CONFIG_FILE.\n\nВсе базовые параметры (WAN-мост, ZFS-хранилище, IP-прокси) будут стерты, а Мастер настройки запустится заново.\n\nПродолжить?" 10 70; then
            ...
            sleep 1
            # Магия Bash: подменяем текущий процесс новым запуском самого себя
            exec "$0" "$@"
        fi
        continue
    fi
    ...
    if [ "$ACTION" == "utils" ]; then
        ...
        case $UTIL_ACTION in
            ...
            download)
                ...
                if [ -n "$YA_URL" ]; then
                    # 🐾 БАГИР: Спрашиваем, как назвать подпапку для идеального порядка
